...
class MOSSSFTDataset(Dataset):
    ...
    def load_data(self):
        ...
        if os.path.exists(data_file) and os.path.exists(no_loss_spans_file):
            self.data = torch.load(data_file, map_location="cpu", weights_only=False)
            self.no_loss_spans = torch.load(no_loss_spans_file, map_location="cpu", weights_only=False)
        else:
